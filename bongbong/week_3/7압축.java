...
신입사원 어피치는 압축 전의 정보를 완벽하게 복원 가능한 무손실 압축 알고리즘을 구현하기로 했다.
...
class Solution {
    public int[] solution(String msg) {
        ...
        while (idx + 1 < msg.length()) {
            ...
            idx += pre.length(); // 현재 문자열 길이만큼 더해서 인덱스 증가 *****

        }

        // 제일 끝의 문자인 경우 바로 answer에 저장
        if (idx == msg.length()-1) {
            answer.add(map.get(msg.charAt(idx) + ""));
        }
