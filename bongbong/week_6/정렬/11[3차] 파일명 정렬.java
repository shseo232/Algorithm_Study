...
class Solution {
    public String[] solution(String[] files) {
        ...
        // Head, Number를 각각 Map에 저장, Map<integer,String>, Map<Integer,Integer>
        for (int i = 0; i < files.length; i++) {
            ...
            for (int j = 0; j < file.length(); j++) {
                ...
                if (Character.isDigit(c)) {
                    ...
                    break; // 숫자 뒤에 문자가 나오면 종료
                }
            }

            headMap.put(i, head.toString().toUpperCase()); // 모두 대문자로 바꿔서 저장
            numberMap.put(i, Integer.parseInt(number.toString()));

        }
        // Comparator 사용해서 HEAD(문자)가 같은경우 NUMBER(숫자) 비교, NUMBER까지 같은 경우 -> 입력 순서가 그대로 유지
        List<Integer> keyList = new ArrayList<>(headMap.keySet());

        Collections.sort(keyList, (s1, s2) -> {
            if (headMap.get(s1).equals(headMap.get(s2))) {
                return numberMap.get(s1).compareTo(numberMap.get(s2));
            }
            return headMap.get(s1).compareTo(headMap.get(s2));
        });
