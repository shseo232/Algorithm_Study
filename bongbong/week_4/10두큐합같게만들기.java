...
class Solution {
    public int solution(int[] queue1, int[] queue2) {
        int answer = 0;
        long totalSum = 0; // 10^9 을 최대 두번 더할 수 있으므로 int 범위 초과할 수 있음
        long q1Sum = 0;
        ...
        for (int i = 0; i < queue1.length; i++) {
            ...
            q1Sum += queue1[i]; // 매번 합을 구하지 않는 아이디어
            q1.offer(queue1[i]);
            q2.offer(queue2[i]);
        }
        ...
        while (q1Sum != target) {
            if (answer > (queue1.length + queue2.length) * 2) { // queue를 완전히 서로 바꿨는데도 같아지지 않은 경우
                return -1;
            }
